# Reconcile approved proposals into the spending ledger
...
**Validated against:** `src/lib/server/reconcile.ts`, `src/lib/spending/index.ts`
...
## Approving the whole proposal adds every row and marks it applied
...
## Dropping a row during review keeps it out of the ledger
...
## Re-categorising a row during review is honoured
...
## A row that wasn't part of this proposal is rejected
...
## A row smuggled in from another import is rejected

- **When** the user approves row "t-1" but re-tagged as imported from item "other"
- **Then** reconciliation is rejected
- **And** the spending ledger is still empty

## Reconciling the same proposal twice doesn't double-count
