...
// `cadenceLabel` now lives in the recurrence engine (`@/lib/buckets`) so every
// Component that speaks the `{ freq, interval }` cadence language shares one label
// vocabulary; re-exported here so spending's public API is unchanged.
export { cadenceLabel } from "@/lib/buckets";
